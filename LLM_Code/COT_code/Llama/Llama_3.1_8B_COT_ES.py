...
XCT_DE = pd.read_json("/home/sbosch/Experiment/ml-kg-mt/data/xct/references/all/es_ES.jsonl", lines = True)
...
Translate from English to Spanish (Spain).
Output only the translation.
Preserve named entities accurately.
Do not add, omit, or explain anything.
Give the output in one sentence only.
Output just the translation and no analysis and/or steps.
"""
}
,
    {
...
- Step 4: María will be translated as Maria and the Guy as the Guy.
Spanish (Spain): María estaba hablando de The Guy.
...
- Step 4: Galileo will be Galileo and Alps will be Alpes.
Spanish (Spain): ¿Cuándo cruzó Galileo los Alpes?
...
- Step 4: Rebels will be translated as Rebeldes, Star Wars as Star Wars and Death Star as la Estrella de la Muerte.
Spanish (Spain): ¿Cómo destruyeron los Rebeldes de Star Wars la Estrella de la Muerte?
...
df_translation.to_csv("translation_COT_LLAMA_ES.csv", index=False)
